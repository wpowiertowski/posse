...
Bluesky uses session strings for authentication instead of static access tokens.

1. **Generate a session string** using the helper script:
   ```bash
   python scripts/get_bluesky_session.py your.handle.bsky.social your-app-password
   ```
   **Note**: Use an [app password](https://bsky.app/settings/app-passwords) instead of your main password for better security.

2. **Store the session string**:
   ```bash
   mkdir -p secrets
   echo "your_session_string" > secrets/bluesky_main_access_token.txt
   ```

3. **Update `config.yml`** with your Bluesky account configuration (see Basic Configuration above)
...
**Important**: Session strings expire when you change your password. If authentication fails, regenerate a new session string.
...
- ✅ Multi-account support for Mastodon and Bluesky
- ✅ Bluesky authentication and posting
- ✅ Automated Docker Hub publishing

**In Progress**:
- 🚧 Mastodon posting integration with Ghost webhook flow
- 🚧 Bluesky posting integration with Ghost webhook flow
