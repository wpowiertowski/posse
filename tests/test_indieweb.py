...
class TestGetWebmentionConfig:
    ...
    def test_legacy_indieweb_config_converted(self):
        """Test that legacy 'indieweb' config key is converted to new format."""
        config = {
            "indieweb": {
                "enabled": True,
                "news": {
        ...
        assert result["targets"][0]["name"] == "IndieWeb News"
        assert result["targets"][0]["endpoint"] == "https://news.indieweb.org/en/webmention"
        assert result["targets"][0]["target"] == "https://news.indieweb.org/en"
        assert result["targets"][0]["tag"] == "indiewebnews"
        assert result["targets"][0]["timeout"] == 30

    def test_legacy_indieweb_config_defaults(self):
        """Test that legacy config with minimal settings uses IndieWeb News defaults."""
        config = {
            "indieweb": {
                "enabled": True,
            }
        ...
        assert result["enabled"] is True
        # No 'news' key → no targets generated
        assert result["targets"] == []

    def test_legacy_indieweb_config_news_defaults(self):
        """Test that legacy config with empty news block uses defaults."""
        config = {
            "indieweb": {
                "enabled": True,
                "news": {},
    ...
    def test_new_config_takes_precedence_over_legacy(self):
        """Test that new 'webmention' key takes precedence over legacy 'indieweb'."""
        config = {
            "webmention": {
                "enabled": True,
                "targets": [
        ...
        assert result["enabled"] is True
        assert len(result["targets"]) == 1
        assert result["targets"][0]["name"] == "Custom Target"

    def test_legacy_indieweb_disabled(self):
        """Test that legacy config respects enabled=false."""
        config = {
            "indieweb": {
                "enabled": False,
                "news": {
...
class TestWebmentionClientLegacyConfig:
    """Test suite for WebmentionClient with legacy indieweb configuration."""

    @patch("indieweb.webmention.requests.post")
    def test_from_config_with_legacy_indieweb_key(self, mock_post):
        """Test creating client from legacy 'indieweb' config key."""
        mock_response = MagicMock()
        mock_response.ok = True
        mock_response.status_code = 200
        ...
        # Verify the client can actually send webmentions
        results = client.send_for_post("https://blog.example.com/my-post", ["indiewebnews"])
        ...
        call_args = mock_post.call_args
        assert call_args[0][0] == "https://news.indieweb.org/en/webmention"
        assert call_args[1]["data"]["source"] == "https://blog.example.com/my-post"
        assert call_args[1]["data"]["target"] == "https://news.indieweb.org/en"
